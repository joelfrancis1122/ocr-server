// src/repositories/tesseractAadhaarOcrRepository.ts
import Tesseract from "tesseract.js";
import { AadhaarOcrRepository, AadhaarOcrResult } from "./aadhaarOcrRepository";

export class TesseractAadhaarOcrRepository implements AadhaarOcrRepository {
  async extractOcrTexts(frontBuffer: Buffer, backBuffer: Buffer): Promise<{ front: string; back: string }> {
    ...
    return { front: ocrResult1.data.text, back: ocrResult2.data.text };
  }

  async parseAadhaarData(frontText: string, backText: string): Promise<AadhaarOcrResult> {
    ...
    if (frontText.search(addressIndicatorRegex) !== -1 && backText.search(addressIndicatorRegex) === -1) {
      console.log(frontText.search(addressIndicatorRegex), "1111111111111");
      console.log(backText.search(addressIndicatorRegex), "222222222222");
      backOCR = frontText;
      frontOCR = backText;
    } else if (backText.search(addressIndicatorRegex) !== -1 && frontText.search(addressIndicatorRegex) === -1) {
      console.log(frontText.search(addressIndicatorRegex), "-----11111");
      console.log(backText.search(addressIndicatorRegex), "-----------222222");
      backOCR = backText;
      frontOCR = frontText;
    } else {
